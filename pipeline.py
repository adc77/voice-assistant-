...
from pydub import AudioSegment, effects  # For pitch and speed adjustment

# Recording and directory settings
FORMAT = pyaudio.paInt16
CHANNELS = 1
RATE = 16000
CHUNK = 1024
...
def record_audio(filename):
    ...
    while not interrupted:
        data = stream.read(CHUNK)
        if vad.is_speech(data, RATE):  # Apply VAD
            frames.append(data)
...
def transcribe_audio(audio_file):
    ...
    transcription_index = max(
        [int(f[len('transcription'):-4]) for f in os.listdir(TRANSCRIPTION_DIR) if f.startswith("transcription") and f.endswith(".txt")],
        default=0
    ) + 1
...
def generate_response(transcription_file):
    ...
    chat_completion = client.chat.completions.create(
        messages=[
            {"role": "system", "content": "You are a concise and focused assistant. Limit responses to 2 sentences."},
            {"role": "user", "content": user_message}
        ],
        model="llama3-8b-8192",
        temperature=0.2,
    ...
    response_content = chat_completion.choices[0].message.content
    response_content = '. '.join(response_content.split('. ')[:2]) + '.'  # Restrict to 2 sentences
...
def text_to_speech(response_file, pitch_shift=0, speed_change=1.0, voice_type="default"):
    processor = SpeechT5Processor.from_pretrained("microsoft/speecht5_tts")
    model = SpeechT5ForTextToSpeech.from_pretrained("microsoft/speecht5_tts")
    vocoder = SpeechT5HifiGan.from_pretrained("microsoft/speecht5_hifigan")
    ...
    embeddings_dataset = load_dataset("Matthijs/cmu-arctic-xvectors", split="validation")
    speaker_embeddings = torch.tensor(embeddings_dataset[7306 if voice_type == "male" else 7315]["xvector"]).unsqueeze(0)
    ...
    if pitch_shift != 0:
        audio = effects.pitch_shift(audio, pitch_shift)
    if speed_change != 1.0:
...
def main():
    ...
    transcription_file = transcribe_audio(output_filename)
    response_file = generate_response(transcription_file)
    audio_output_file = text_to_speech(response_file, pitch_shift=0, speed_change=1.0, voice_type="default")
